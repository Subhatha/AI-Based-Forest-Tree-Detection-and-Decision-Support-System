...
def detect_trees(input_folder, output_folder):
    ...
    # Increased distance helps maintain IDs longer
    ...
    for image_path in image_files:
        ...
        for object_id, (cx, cy) in tracked_objects.items():
            ...
            x1, y1, x2, y2, conf = detection_data[
                closest_index
            ]
            ...
            decision = (
                "CUT"
                if box_height < CUT_HEIGHT_THRESHOLD
                else "KEEP"
            )
            ...
            if nearest_id is not None:
                ...
                label = (
                    f"Tree {object_id}: {decision} | "
                    f"Near T{nearest_id}: "
                    f"{nearest_distance:.0f}px"
                )
            ...
            # Count only once per unique tracked ID
